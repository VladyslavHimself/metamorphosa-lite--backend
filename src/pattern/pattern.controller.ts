...
export class PatternController {
  ...
  @ApiParam({ name: 'topic', example: 'excercise', description: 'topic for pattern' })
  ...
  async getAllPatterns(
    ...
    return await this.patternService.getAllPatterns(topic.toLowerCase(), userId);
  }
  ...
  @ApiResponse({ status: 200, type: PatternEntity })
  @Post(':topic')
  async createPattern(@Param('topic') topic: string, @Body() body: {}, @GetUserId() userId: number): Promise<PatternEntity> {
      return await this.patternService.createPattern(topic.toLowerCase(), body, userId)
  }
  ...
  @ApiResponse({ status: 200, type: PatternEntity })
  @Put(':id')
  async editPattern(@Param('id') id: number, @Body() body: {}, @GetUserId() userId: number): Promise<any> {
